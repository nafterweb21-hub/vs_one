    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 pb-6 border-b border-zinc-200 dark:border-zinc-800">
        <div>
          <div className="flex items-center gap-2 text-xs text-zinc-400 dark:text-zinc-500 font-semibold tracking-wider uppercase mb-1">
            ...
            <span>/</span>
            <span className="text-zinc-500 dark:text-zinc-400">Sales Orders</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Sales Orders</h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">Manage customer sales orders and items.</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 rounded-xl shadow-sm">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors"
          />
        </div>
        <div className="sm:w-48">
          <select
            ...
            className="w-full px-3 py-2 text-sm bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-zinc-700 dark:text-zinc-300 transition-colors"
          >
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-12 text-center shadow-sm">
          ...
          <p className="text-zinc-600 dark:text-zinc-400 font-semibold">No sales orders found.</p>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left whitespace-nowrap">
              <thead className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-50/50 dark:bg-zinc-900/50 uppercase tracking-wider border-b border-zinc-200 dark:border-zinc-800">
                <tr>
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors group">
                    <td className="px-6 py-4 font-bold text-zinc-900 dark:text-white">
                      <Link href={`/dashboard/sales/sales-order/${order.id}`} className="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                        {order.orderNo}
                        {order.revision > 0 && <span className="ml-1 text-xs text-zinc-500 font-normal">v{order.revision}</span>}
                      </Link>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right font-mono font-semibold text-zinc-900 dark:text-white">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${
                          order.status === "Confirmed" ? "bg-emerald-500" : order.status === "Closed" ? "bg-zinc-400" : "bg-amber-500"
                        }`} />
                        {order.status}
                      </span>
                    </td>
