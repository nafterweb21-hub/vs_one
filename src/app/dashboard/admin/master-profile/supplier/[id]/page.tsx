...
export default function ManageSupplierPage({ params }: PageProps) {
  ...
  const [detailTab, setDetailTab] = useState<"contacts" | "addresses">("contacts");
  ...
  return (
    <div className="w-full space-y-5 bg-white text-black">
      {/* Navigation & Header */}
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="rounded-md border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-sm font-bold text-zinc-700">
      ...
      {/* Main Grid Layout - Left: General Info, Right: Contacts / Addresses */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        
        {/* LEFT COLUMN: General Details & Remarks (col-span-5) */}
        <div className="lg:col-span-5 flex flex-col gap-4 lg:border-r lg:border-zinc-100 pr-0 lg:pr-6 bg-white">
          <h3 className="text-base font-bold text-black border-b border-zinc-100 pt-[6px] pb-[10px]">
            General Info
          </h3>

          <div className="flex flex-col gap-1">
            <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Supplier Code</span>
            <div className="rounded-lg bg-zinc-50 border border-zinc-100 px-3 py-2 text-base font-bold text-zinc-800">
              {supplierDetail.supplierCode}
            </div>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Supplier Name</span>
            <div className="rounded-lg bg-zinc-50 border border-zinc-100 px-3 py-2 text-base font-medium text-zinc-800">
              {supplierDetail.supplierName}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              ...
              disabled={isPending}
              className="rounded-lg glossy-button-blue px-4 py-2 text-sm font-bold text-white shadow-xs cursor-pointer"
            >
              Save Remarks
            </button>
            {remarksSuccess && (
        ...
        {/* RIGHT COLUMN: Contacts and Addresses with Tabs (col-span-7) */}
        <div className="lg:col-span-7 flex flex-col pl-0 bg-white min-h-[460px]">
          {/* Tab Selector */}
          <div className="flex border-b border-zinc-200 mb-4 shrink-0">
            <button
              onClick={() => {
              ...
              className={`pr-4 py-2.5 text-sm font-bold border-b-2 transition-all cursor-pointer ${
                detailTab === "contacts"
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
            ...
            {detailTab === "contacts" && (
              <div className="flex-1 flex flex-col gap-4">
                <div className="flex justify-between items-center shrink-0">
                  ...
                  {!isContactFormOpen && (
                    <button
                      ...
                      className="rounded-lg glossy-button-blue px-3.5 py-1.5 text-xs font-bold text-white shadow-xs cursor-pointer"
                    >
                      Add Contact Person
                    </button>
                  )}
                ...
                {isContactFormOpen ? (
                  <form onSubmit={handleContactSubmit} className="border border-blue-100 bg-blue-50/10 p-4 rounded-xl space-y-4 shrink-0">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-[11px] font-bold text-zinc-600">Contact Person <span className="text-red-500">*</span></label>
                        <input
                          type="text"
                          required
                          value={contactName}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[11px] font-bold text-zinc-600">Tel No</label>
                        <input
                          type="text"
                          value={contactTel}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[11px] font-bold text-zinc-600">Mobile No</label>
                        <input
                          type="text"
                          value={contactMobile}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[11px] font-bold text-zinc-600">Fax No</label>
                        <input
                          type="text"
                          value={contactFax}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[11px] font-bold text-zinc-600">Designation</label>
                        <input
                          type="text"
                    ...
                    {!contactEditId && (
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="manageContactDefault" className="text-xs font-semibold text-zinc-700">
                    ...
                    <div className="flex items-center gap-2 justify-end pt-1">
                      <button
                        ...
                        className="rounded-lg glossy-button-white px-3.5 py-1.5 text-xs font-semibold cursor-pointer"
                      >
                        Cancel
                      </button>
                      <button
                        type="submit"
                        disabled={isPending}
                        className="rounded-lg glossy-button-blue px-3.5 py-1.5 text-xs font-bold text-white cursor-pointer"
                      >
                        Save Contact
                      </button>
                    </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border border-dashed border-zinc-200 rounded-lg">
                    <p className="text-sm font-semibold text-zinc-800">No contacts defined</p>
                    <p className="text-xs text-zinc-550 mt-0.5">Click Add Contact Person to define supplier contacts.</p>
                  </div>
                ) : (
                  <div className="flex-1 overflow-x-auto border border-zinc-200 rounded-lg">
                    <table className="w-full text-left border-collapse text-[11px]">
                      <thead>
                        <tr className="border-b border-zinc-200 bg-zinc-50 font-bold text-zinc-700 sticky top-0">
                          <th className="px-3 py-2.5">Name</th>
                          <th className="px-3 py-2.5">Phone Details</th>
                      ...
                      <tbody className="divide-y divide-zinc-100">
                        {supplierDetail.contactPersons.map((contact) => (
                          <tr key={contact.id} className="hover:bg-zinc-50 transition-colors">
                            <td className="px-3 py-2.5 font-bold text-black">{contact.contactPersonName}</td>
                            <td className="px-3 py-2.5 text-zinc-800 space-y-0.5">
                            ...
                            <td className="px-3 py-2.5 text-zinc-850 truncate max-w-[120px]" title={contact.email || undefined}>
                              {contact.email || "—"}
                            </td>
                            <td className="px-3 py-2.5 text-zinc-800">{contact.designation || "—"}</td>
                            ...
                            <td className="px-3 py-2.5 text-right">
                              <div className="flex items-center justify-end gap-1">
                                <button
                                  ...
                                  className="rounded-md border border-zinc-200 bg-white hover:bg-zinc-55 px-2 py-1 text-[10px] font-medium text-zinc-800 cursor-pointer"
                                >
                                  Edit
                                </button>
                                <button
                                ...
                                <button
                                  ...
                                  className="rounded-md border border-red-200 bg-red-50 hover:bg-red-100 px-2 py-1 text-[10px] font-bold text-red-700 cursor-pointer"
                                >
                                  Del
                                </button>
                              </div>
            ...
            {detailTab === "addresses" && (
              <div className="flex-1 flex flex-col gap-4">
                ...
                {isAddressFormOpen ? (
                  <form onSubmit={handleAddressSubmit} className="border border-blue-100 bg-blue-50/10 p-4 rounded-xl space-y-4 shrink-0">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-blue-700">
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-[11px] font-bold text-zinc-650">Address Details <span className="text-red-500">*</span></label>
                      <textarea
                        required
                        rows={3}
                        value={addressText}
                        onChange={(e) => setAddressText(e.target.value)}
                        className="rounded-lg glossy-input px-3 py-2 text-xs resize-none w-full"
                    ...
                    {!addressEditId && (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="h-4 w-4 rounded border-zinc-300 text-blue-600"
                        />
                        <label htmlFor="manageAddressDefault" className="text-xs font-semibold text-zinc-700">
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border border-dashed border-zinc-200 rounded-lg">
                    <p className="text-sm font-semibold text-zinc-800">No addresses defined</p>
                    <p className="text-xs text-zinc-550 mt-0.5">Click Add Address to define supplier shipping/billing locations.</p>
                  </div>
                ) : (
                  <div className="flex-1 overflow-x-auto border border-zinc-200 rounded-lg">
                    <table className="w-full text-left border-collapse text-[11px]">
                      ...
                      <tbody className="divide-y divide-zinc-100">
                        {supplierDetail.addresses.map((addr) => (
                          <tr key={addr.id} className="hover:bg-zinc-50 transition-colors">
                            <td className="px-3 py-2.5 text-zinc-850 whitespace-pre-wrap max-w-sm">
                              {addr.address}
                            </td>
                            <td className="px-3 py-2.5 text-center">
                              {addr.isDefault ? (
