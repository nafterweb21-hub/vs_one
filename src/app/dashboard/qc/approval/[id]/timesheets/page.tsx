...
import ParameterDetailDrawer from "@/app/dashboard/production/work-order/components/ParameterDetailDrawer";

export default async function QcApprovalTimesheetsPage({
  params,
}: {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href="/dashboard/qc/approval"
          className="p-2 hover:bg-slate-100 rounded-lg text-slate-500 transition-colors"
        >
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-800">QC Approval: {workOrder.workOrderNo}</h1>
      ...
      <div className="border-b border-slate-200">
        <nav className="-mb-px flex space-x-8">
          ...
          <Link
            href={`/dashboard/qc/approval/${id}/routing`}
            className="border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 py-4 px-1 text-sm font-medium"
          >
            In-Process & Routing
          </Link>
        ...
        <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Read-only view of production timesheets and parameters.
            </p>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {rows.map((ts: any) => (
                  <tr key={ts.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      {ts.qcStatus === 'Approved' ? (
                        <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">Approved</span>
                      ) : ts.qcStatus === 'Rejected' ? (
                        <div className="flex flex-col items-center">
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-700">Rejected</span>
                          {ts.qcRemark && <span className="text-[10px] text-rose-600 mt-1 truncate max-w-[100px]" title={ts.qcRemark}>{ts.qcRemark}</span>}
                        </div>
                      ) : (
                        <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-700">Pending</span>
                      )}
                    </td>
                  </tr>
                ))}
