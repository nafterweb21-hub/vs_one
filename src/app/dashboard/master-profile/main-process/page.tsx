...
export default function MainProcessProfilePage() {
  const [items, setItems] = useState<MainProcess[]>([]);
  ...
  const loadData = async () => {
    ...
    if (res.success && res.data) {
      setItems(res.data as MainProcess[]);
    } else {
      setErrorMsg(res.error || "Failed to load main processes.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await toggleMainProcessStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const filteredItems = items.filter(
    (m) =>
      m.process.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="flex-1 flex flex-col p-6 space-y-6 max-w-7xl mx-auto w-full bg-blue-50 text-blue-900 ">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-blue-500 mt-1">
            Maintain list of routing process categories.
          </p>
        </div>

        <Link
          href="/dashboard/master-profile/main-process/new"
      ...
      <div className="rounded-xl border border-blue-200 bg-white p-4">
        <div className="relative max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by main process or remark..."
            value={searchQuery}
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-blue-900 ">Error loading main processes</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-blue-900 ">No main processes found</p>
            <p className="text-sm text-blue-500 mt-1">
              {searchQuery ? "No matches for your criteria." : "Create your first Main Process Profile to begin."}
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/dashboard/master-profile/main-process/${item.id}/edit`}
