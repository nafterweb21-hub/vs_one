...
export default function EmployeeProfilePage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast("success", `Employee "${employeeToDelete.name}" has been deleted successfully.`);
          ...
          <div className="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-6 text-left align-middle shadow-2xl transition-all duration-300 animate-in fade-in zoom-in-95">
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-tr from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 text-sm font-semibold text-white shadow-md shadow-rose-500/10 hover:shadow-lg hover:shadow-rose-500/20 active:scale-[0.98] transition-all disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
              >
                {isDeleting ? (
                  <>
