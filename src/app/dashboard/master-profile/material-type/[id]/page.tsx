...
export default function EditMaterialTypePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (res.success && res.data) {
        ...
        setErrorMsg(res.error || "Failed to load material type details");
      }
      setIsLoading(false);
    };
    loadData();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await updateMaterialTypeProfile(id, { remark });

    if (res.success) {
      router.push("/dashboard/master-profile/material-type");
    } else {
