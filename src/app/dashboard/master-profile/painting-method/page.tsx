...
import {
  getPaintingMethodProfiles,
  togglePaintingMethodStatus,
  deletePaintingMethodProfile,
} from "./actions";
...
export default function PaintingMethodProfilePage() {
  const [items, setItems] = useState<PaintingMethodProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    if (res.success && res.data) {
      setItems(res.data as PaintingMethodProfile[]);
    } else {
      setErrorMsg(res.error || "Failed to load painting methods.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await togglePaintingMethodStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this painting method?")) {
      startTransition(async () => {
        const res = await deletePaintingMethodProfile(id);
        if (res.success) {
          loadData();
        } else {
          alert(res.error || "Failed to delete painting method.");
        }
      });
    }
  };
    ...
    <div className="flex-1 flex flex-col p-6 space-y-6 max-w-7xl mx-auto w-full bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">
            Painting Method Profile
          </h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Maintain the master list of painting methods used in certificate of conformity.
          </p>
        </div>

        <Link
          href="/dashboard/master-profile/painting-method/new"
      ...
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
        <div className="relative max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by method or remark..."
            value={searchQuery}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-zinc-900 dark:text-zinc-100">Error loading painting methods</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-zinc-900 dark:text-zinc-100">No painting methods found</p>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
              {searchQuery ? "No matches for your criteria." : "Create your first Painting Method to begin."}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                  ...
                  <th className="px-6 py-4">Painting Method</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/dashboard/master-profile/painting-method/${item.id}/edit`}
