...
import {
  getJointProfileItems,
  toggleJointProfileStatus,
  deleteJointProfileItem,
} from "./actions";
...
export default function JointProfilePage() {
  const [items, setItems] = useState<JointProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    if (res.success && res.data) {
      setItems(res.data as JointProfile[]);
    } else {
      setErrorMsg(res.error || "Failed to load joint profiles.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await toggleJointProfileStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this joint profile?")) {
      startTransition(async () => {
        const res = await deleteJointProfileItem(id);
        if (res.success) {
          loadData();
        } else {
          alert(res.error || "Failed to delete joint profile.");
        }
      });
    }
  };
  ...
  const filteredItems = items.filter((m) => {
    const matchesSearch =
      m.joint.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="flex-1 flex flex-col p-6 space-y-6 max-w-7xl mx-auto w-full bg-blue-50 text-blue-900 ">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-blue-500 mt-1">
            Maintain the master list of welding joint types used in work orders.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-blue-200 bg-white p-4 flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by joint name or remark..."
            value={searchQuery}
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-blue-900 ">Error loading joint profiles</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-blue-900 ">No joint profiles found</p>
            <p className="text-sm text-blue-500 mt-1">
              {searchQuery || statusFilter !== "All" ? "No matches for your criteria." : "Create your first Joint Profile to begin."}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="border-b border-blue-200 bg-blue-50 text-xs font-bold uppercase tracking-wider text-blue-500 ">
                  ...
                  <th className="px-6 py-4">Joint</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/dashboard/master-profile/joint/${item.id}/edit`}
