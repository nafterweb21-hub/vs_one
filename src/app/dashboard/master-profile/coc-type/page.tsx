...
import {
  getCocTypesList,
  toggleCocTypeStatus,
  deleteCocTypeProfile,
} from "./actions";

interface CocTypeProfile {
  id: string;
  type: string;
  remark: string | null;
  status: string;
...
export default function CocTypeProfilePage() {
  const [items, setItems] = useState<CocTypeProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    const res = await getCocTypesList();

    if (res.success && res.data) {
      setItems(res.data as CocTypeProfile[]);
    } else {
      setErrorMsg(res.error || "Failed to load COC types.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await toggleCocTypeStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this COC Type?")) {
      startTransition(async () => {
        const res = await deleteCocTypeProfile(id);
        if (res.success) {
          loadData();
        } else {
          alert(res.error || "Failed to delete COC type.");
        }
      });
    }
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-blue-500 mt-1">
            Maintain the master list of COC types.
          </p>
        </div>

        <Link
          href="/dashboard/master-profile/coc-type/new"
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-blue-900">Error loading COC types</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-blue-900">No COC types found</p>
            <p className="text-sm text-blue-500 mt-1">
              {searchQuery ? "No matches for your criteria." : "Create your first COC Type to begin."}
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/dashboard/master-profile/coc-type/${item.id}`}
