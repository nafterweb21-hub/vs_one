...
import {
  getCocTypeDetail,
  updateCocTypeProfile,
} from "../actions";

export default function EditCocTypePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      const res = await getCocTypeDetail(id);
      if (res.success && res.data) {
        ...
        setErrorMsg(res.error || "Failed to load COC type details");
      }
      setIsLoading(false);
    };
    loadData();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await updateCocTypeProfile(id, { remark });
      ...
      <div className="bg-white rounded-2xl border border-blue-200 shadow-sm overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="space-y-4">
            <div>
              <label htmlFor="type" className="block text-sm font-semibold text-blue-900 mb-1.5 opacity-70">
                COC Type
              </label>
              <input
                id="type"
              ...
              <p className="text-xs text-blue-500 mt-1.5">
                COC type cannot be changed once created.
              </p>
            </div>
