...
import {
  getDepartmentProfileItems,
  toggleDepartmentProfileStatus,
  deleteDepartmentProfileItem,
} from "./actions";
...
export default function DepartmentProfilePage() {
  const [items, setItems] = useState<DepartmentProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    const res = await getDepartmentProfileItems();

    if (res.success && res.data) {
      setItems(res.data as DepartmentProfile[]);
    } else {
      setErrorMsg(res.error || "Failed to load department profiles.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await toggleDepartmentProfileStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const executeDelete = async () => {
    ...
    startTransition(async () => {
      const res = await deleteDepartmentProfileItem(itemToDelete);
      if (res.success) {
        loadData();
      } else {
        alert(res.error || "Failed to delete department profile.");
      }
    ...
    <div className="flex-1 flex flex-col p-6 space-y-6 max-w-7xl mx-auto w-full bg-blue-50 text-blue-900 ">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-blue-500 mt-1">
            Maintain the master list of departments.
          </p>
        </div>

        <Link
          href="/dashboard/master-profile/department/new"
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-blue-900 ">Error loading department profiles</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-blue-900 ">No department profiles found</p>
            <p className="text-sm text-blue-500 mt-1">
              {searchQuery || statusFilter !== "All" ? "No matches for your criteria." : "Create your first Department Profile to begin."}
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {/* 
                        <Link
                          href={`/dashboard/master-profile/department/${item.id}`}
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-blue-950/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl border border-blue-100 w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-blue-950 mb-2">Delete Profile</h3>
              <p className="text-sm text-blue-600">
                Are you sure you want to delete this department profile? This action cannot be undone.
              </p>
            </div>
            <div className="bg-blue-50/50 px-6 py-4 flex gap-3 justify-end border-t border-blue-100">
              <button
                type="button"
                onClick={() => {
                  setDeleteModalOpen(false);
                ...
                className="rounded-lg px-4 py-2 text-sm font-semibold text-blue-700 bg-white border border-blue-200 hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-lg px-4 py-2 text-sm font-semibold text-white bg-rose-600 hover:bg-rose-500 shadow-sm focus:outline-none focus:ring-2 focus:ring-rose-500/50 transition-colors"
              >
                Delete
              </button>
            </div>
