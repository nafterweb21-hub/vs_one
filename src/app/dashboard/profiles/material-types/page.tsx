...
function MaterialTypesPage() {
  ...
  const [categories, setCategories] = useState<MaterialType[]>([]);
  ...
  useEffect(() => {
    if (toastType === "created") {
      triggerToast("New Material Type created successfully.");
      router.replace("/dashboard/profiles/material-types");
    } else if (toastType === "updated") {
      triggerToast("Material Type updated successfully.");
      router.replace("/dashboard/profiles/material-types");
    }
  }, [toastType, router]);
  ...
  const handleCreateOpen = () => {
    router.push("/dashboard/profiles/material-types/create");
  };

  // Navigate to Edit page
  const handleEditOpen = (type: MaterialType) => {
    router.push(`/dashboard/profiles/material-types/edit/${type.id}`);
  };
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/profiles/material-types/${id}`, {
        method: "PATCH",
  ...
  const handleVoidtype = async (id: string) => {
    if (!confirm("Are you sure you want to VOID this Material Type? Voided records cannot be restored and running audit logs are permanently maintained.")) {
      return;
    }
    ...
    try {
      ...
      triggerToast("Material Type voided successfully.");
    } catch (err: any) {
