...
export default function MaterialCategoriesPage() {
  ...
  useEffect(() => {
    if (toastType === "created") {
      triggerToast("New material category created successfully.");
      router.replace("/dashboard/profiles/material-categories");
    } else if (toastType === "updated") {
      triggerToast("Material category updated successfully.");
      router.replace("/dashboard/profiles/material-categories");
    }
  }, [toastType, router]);
  ...
  const handleCreateOpen = () => {
    router.push("/dashboard/profiles/material-categories/create");
  };

  // Navigate to Edit page
  const handleEditOpen = (category: MaterialCategory) => {
    router.push(`/dashboard/profiles/material-categories/edit/${category.id}`);
  };
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/profiles/material-categories/${id}`, {
        method: "PATCH",
      ...
      triggerToast(`Category status successfully set to ${nextStatus}.`);
    } catch (err: any) {
      alert(err.message || "Could not toggle status.");
    }
  };

  // Void a category (No delete - Void only)
  const handleVoidCategory = async (id: string) => {
    if (!confirm("Are you sure you want to VOID this material category? Voided records cannot be restored and running audit logs are permanently maintained.")) {
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to void category.");

      setCategories((prev) =>
        prev.map((c) => (c.id === id ? { ...c, status: "Void" } : c))
      );
      triggerToast("Material category voided successfully.");
    } catch (err: any) {
      alert(err.message || "Failed to void category.");
    }
  };
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            {activeCategories.length === 0 ? (
              <div className="p-8 text-center text-xs text-zinc-500 dark:text-zinc-400">
                No active material categories configured. Click "Create Material Category" to add one.
              </div>
            ) : (
                      ...
                      <tr key={c.id} className="hover:bg-zinc-50/40 dark:hover:bg-zinc-850/10">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2.5">
                            ...
                            <button
                              onClick={() => handleVoidCategory(c.id)}
                ...
                <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800 text-left">
                  ...
                  <tbody className="divide-y divide-zinc-100 bg-white text-xs dark:divide-zinc-800 dark:bg-zinc-900 text-zinc-600">
                    {voidedCategories.map((c) => (
                      <tr key={c.id}>
                        <td className="px-6 py-2.5 font-semibold">{c.category}</td>
