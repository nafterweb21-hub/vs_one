...
export default function PurchaseRequisitionListPage() {
  ...
  const [historyRows, setHistoryRows] = useState<PurchaseRequisition[]>([]);
  const [historyTitle, setHistoryTitle] = useState("");

  const toastParam = searchParams.get("toast");
  useEffect(() => {
    if (!toastParam) return;
    if (toastParam === "updated") setToast("Purchase Requisition updated successfully.");
    else if (toastParam === "created") setToast("Purchase Requisition created successfully.");
    const t = setTimeout(() => setToast(null), 4000);
    router.replace("/dashboard/purchasing/purchase-requisition");
  ...
  const fetchRows = async () => {
    ...
    try {
      const res = await fetch(
        `/api/purchasing/purchase-requisition?search=${encodeURIComponent(search)}&status=${tab}`,
      );
  ...
  async function callAction(id: string, action: "submit" | "void" | "revise") {
    const res = await fetch(`/api/purchasing/purchase-requisition/${id}/transition`, {
      method: "POST",
  ...
  async function onHistory() {
    if (!selected) return;
    setHistoryTitle(selected.prNo);
    setHistoryOpen(true);
    setHistoryLoading(true);
    try {
      const res = await fetch(
        `/api/purchasing/purchase-requisition/history?prNo=${encodeURIComponent(selected.prNo)}`,
      );
  ...
  async function onCopy() {
    if (!selected) return;
    try {
      const full = await fetch(`/api/purchasing/purchase-requisition/${selected.id}`).then((r) => r.json());
      const payload = {
      ...
      const res = await fetch(`/api/purchasing/purchase-requisition`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="p-6 lg:p-8 space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 pb-6 border-b border-blue-200">
        <div>
          <div className="flex items-center gap-2 text-xs text-blue-400 font-semibold tracking-wider uppercase mb-1">
            ...
            <span>/</span>
            <span className="text-blue-500">Purchase Requisitions</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-blue-900">Purchase Requisition</h2>
              <p className="text-sm text-blue-500 mt-0.5">Create and manage material or non-material purchase requisitions.</p>
            </div>
          </div>
        </div>

        <Link
          href="/dashboard/purchasing/purchase-requisition/new"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-white border border-blue-200 p-4 rounded-xl shadow-sm">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by PR No, Work Order, Requisitioner or Remark..."
            value={search}
      ...
      {loading ? (
        <div className="h-64 flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-sm text-blue-500">Loading requisitions...</p>
        </div>
      ) : errorMsg ? (
        ...
        <div className="bg-white border border-blue-200 rounded-xl p-12 text-center shadow-sm">
          ...
          <p className="text-blue-600 font-semibold">No purchase requisitions found.</p>
          <p className="text-xs text-blue-400 mt-1">Click &quot;New Requisition&quot; to create your first.</p>
        </div>
      ) : (
                ...
                {rows.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-bold text-blue-900">
                      <Link
                        href={`/dashboard/purchasing/purchase-requisition/${r.id}`}
                    ...
                    <td className="px-4 py-3 text-blue-700 font-medium">{r.company?.companyName}</td>
                    ...
                    <td className="px-4 py-3 text-blue-700">{r.requestedBy?.name}</td>
                    <td className="px-4 py-3">
                      <POStatusPill status={r.poStatus} />
                    </td>
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-blue-200 bg-blue-50 text-xs font-bold text-blue-700 uppercase tracking-wider">
                      ...
                      <th className="px-3 py-2.5 text-left">PO Status</th>
                  ...
                  <tbody className="divide-y divide-blue-100">
                    {historyRows.map((h) => (
                      <tr key={h.id} className="hover:bg-blue-50/40">
                        ...
                        <td className="px-3 py-2.5 text-blue-700">{h.requestedBy?.name}</td>
                        <td className="px-3 py-2.5"><StatusPill status={h.status} /></td>
                        <td className="px-3 py-2.5"><POStatusPill status={h.poStatus} /></td>
                        <td className="px-3 py-2.5 text-right">
                          <Link
                            href={`/dashboard/purchasing/purchase-requisition/${h.id}`}
