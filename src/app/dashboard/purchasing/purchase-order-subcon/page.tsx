...
export default function PurchaseOrderSubconListPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [rows, setRows] = useState<PurchaseOrderSubcon[]>([]);
  ...
  const [historyRows, setHistoryRows] = useState<PurchaseOrderSubcon[]>([]);
  const [historyTitle, setHistoryTitle] = useState("");

  const toastParam = searchParams.get("toast");
  useEffect(() => {
    if (!toastParam) return;
    if (toastParam === "updated") setToast("Purchase Order Subcon updated successfully.");
    else if (toastParam === "created") setToast("Purchase Order Subcon created successfully.");
  ...
  const fetchRows = async () => {
    ...
    try {
      const res = await fetch(
        `/api/purchasing/purchase-order-subcon?search=${encodeURIComponent(search)}&status=${tab}`,
      );
      if (!res.ok) throw new Error("Failed to fetch purchase orders subcon");
  ...
  async function callAction(id: string, action: "submit" | "void" | "revise") {
    // Note: Reusing the same transition API for state changes
  ...
  async function onVoid() {
    ...
    if (!confirm(`Void purchase order subcon ${selected.poNo}-R${selected.revision}?`)) return;
  ...
  async function onCopy() {
    if (!selected) return;
    try {
      const full = await fetch(`/api/purchasing/purchase-order-subcon/${selected.id}`).then((r) => r.json());
      const payload = {
        ...full,
        id: undefined,
        poNo: undefined,
      ...
      const res = await fetch(`/api/purchasing/purchase-order-subcon`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      alert(e.message || "Failed to copy purchase order subcon");
    }
  }
    ...
    <div className="p-6 lg:p-8 space-y-6 relative">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-blue-200 rounded-xl p-12 text-center shadow-sm">
          ...
          <p className="text-blue-600 font-semibold">No subcon purchase orders found.</p>
          <p className="text-xs text-blue-400 mt-1">Click &quot;New PO Subcon&quot; to create your first.</p>
        </div>
      ) : (
